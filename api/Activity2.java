...
public class Activity2 {
    ...
    public void getUserInfo() {
        ...
        Response response =
                given().contentType(ContentType.JSON) // Set headers
                        .pathParam("username", "mriganka") // Pass request body from file
        ...
        response.then().body("username", equalTo("mriganka"));
        response.then().body("firstName", equalTo("Mriganka"));
        response.then().body("lastName", equalTo("Case"));
        response.then().body("email", equalTo("mrigankacase@mail.com"));
    ...
    public void deleteUser() throws IOException {
        Response response =
                given().contentType(ContentType.JSON) // Set headers
                        .pathParam("username", "mriganka") // Add path parameter
                        .when().delete(ROOT_URI + "/{username}"); // Send POST request

        // Assertion
        response.then().body("code", equalTo(200));
        response.then().body("message", equalTo("mriganka"));
    }
}